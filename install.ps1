...
# Updated manually
New-Variable -Name 'SCRIPT_SEMVER_VERSION' -Value '1.0.18' -Option Constant
...
New-Variable -Name 'PUBLISH_STRING' -Value '20220218' -Option Constant

# Updated automatically by the CI pipeline
New-Variable -Name 'SCRIPT_COMMIT_SHA' -Value '17a7d28' -Option Constant
...
function getNumericallyHigherVersion {
    ...
    if( $c -like "*rc*" -or $c -like "*tp*"  -or $c -like "*dev*"  -or $c -like "*nightly*"  -or $c -like "*beta*")
    {
        ...
        Foreach ($i in $tmpPkgs)
        {
             if ($i -eq $mainRel)
             {
                 return addLeadingZeroMinorVer($i)
             }
        }
    }
    return addLeadingZeroMinorVer($c)
}

# System.Version truncates leading 0s for minor version. For example, 19.03.xx gets truncated to 19.03.xx
# This function will fix by adding leading 0. We never use name like 19.03.0X, it'll be 19.03.X
function addLeadingZeroMinorVer{
    param( [string] $version )
    # For now, functions, with leading zero are 17.03, 18.06, 19.03
    if ($version -match "^\d{2}\.\d\.\d{1,2}") {
        $version = $version.substring(0,3) + "0" + $version.substring(3, $version.length-3)
    }
    return $version
}
...
function printPackageVersions {
        ...
        $json=(Invoke-WebRequest -Uri $indexUrl).Content|ConvertFrom-Json
